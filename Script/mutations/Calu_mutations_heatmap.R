...
#
# High-impact somatic mutation heatmap across three Calu-1 conditions:
#   000 = naive, 010 = sotorasib, 020 = adagrasib.
#
# NOTE: Calu-1's SNV table is small (~11 rows total across all three
# conditions in the current .rds). The HIGH-impact subset may be very small,
# yielding a short heatmap. That is expected; the script will still render.
#
# Pipeline:
...
#     cd /path/to/PanRAS
#     Rscript Calu_mutations_heatmap.R
#
# Override defaults from the shell:
#     Rscript Calu_mutations_heatmap.R path/to/calu_cnv_snv_sv.rds out_dir TITLE
# =============================================================================
...
rds_path   <- if (length(args) >= 1) args[1] else file.path(INPUT_DIR, "calu_cnv_snv_sv.rds")
out_dir    <- if (length(args) >= 2) args[2] else file.path(FIGURES_DIR, "mutations")
TITLE      <- if (length(args) >= 3) args[3] else "CALU"
...
#   "TP53MYCSRSF" : TP53 / MYC / SRSF1..SRSF12
# Symbols are matched case-insensitively against gene_sym (resolved earlier
# from gene_name -> gene_id_nv). TRP53 is included to mirror the mouse alias
...
#
# Gene-symbol fallback cascade: gene_name -> gene_id_nv -> "?".
...
required_cols <- c("ID", "IMPACT", "TUMOR_AF", "condition",
                   "gene_name", "Codons", "Protein_position", "Consequence")
...
row_labels <- vapply(muts, function(id) {
  r <- first_per_id[id, ]
  nice_label(r$gene_name, r$gene_id_nv,
...
# Per-row gene symbol (same fallback cascade as nice_label: gene_name ->
# gene_id_nv -> NA). Used to flag rows in the RAS gene family.
...
# against gene_sym, which already resolved from gene_name -> gene_id_nv).
# -----------------------------------------------------------------------------
