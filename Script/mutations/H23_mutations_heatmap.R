...
#
# High-impact somatic mutation heatmap across three H23 conditions:
...
#     cd /path/to/PanRAS
#     Rscript H23_mutations_heatmap.R
#
# Override defaults from the shell:
#     Rscript H23_mutations_heatmap.R path/to/h23_cnv_snv_sv.rds out_dir TITLE
# =============================================================================
...
rds_path   <- if (length(args) >= 1) args[1] else file.path(INPUT_DIR, "h23_cnv_snv_sv.rds")
out_dir    <- if (length(args) >= 2) args[2] else file.path(FIGURES_DIR, "mutations")
TITLE      <- if (length(args) >= 3) args[3] else "H23"
