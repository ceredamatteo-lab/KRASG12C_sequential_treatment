## ***************************
##
## Script name: caks_h2301_processing_vcf_files_SNV.R
...
# data -----
model = 'caks-h2301'
samples = c('caks-h2301-50-050-000_downsampledNormal', 'caks-h2301-50-050-010_downsampledNormal', 'caks-h2301-50-050-020_downsampledNormal')
control = 'mgpf-af001-10-050-000'
is_mouse = FALSE
...
mutect.path = paste0(DATA_DIR, "Results/sarek/")
file_suffix_mutect2_VEP = ".mutect2.filtered_VEP.ann.vcf.gz"

strelka.path = paste0(DATA_DIR, "Results/sarek/")
...
for (sample in samples){
  ...
  file_path_mutect2 = paste0(mutect.path, sample, "_vs_", control, '/annotation/mutect2/', sample, "_vs_", control, '/', sample, "_vs_", control, file_suffix_mutect2_VEP )
...
saveRDS(mutect2_df, paste0(DATA_DIR, "Rdata/", model, '_vs_', control, '__', 'mutect2_no_filters.rds'))
saveRDS(strelka_df, paste0(DATA_DIR, "Rdata/", model, '_vs_', control, '__', 'strelka_no_filters.rds'))
...
saveRDS(overlap_df, paste0(DATA_DIR, "Rdata/", model,  '_vs_', control, '__', 'filtered_overlap_mutect2_strelka.rds'))
...
lapply(unique(selected$sample), function(x){length(unique(selected$ID[selected$sample==x]))})
saveRDS(selected, paste0(DATA_DIR, "Rdata/", model, '_vs_', control, '__', 'filtered_overlap_mutect2_strelka_PCgenes_autochrom.rds'))
...
lapply(unique(selected$sample), function(x){length(unique(selected$ID[selected$sample==x]))})
saveRDS(selected, paste0(DATA_DIR, "Rdata/", model, '_vs_', control, '__', 'filtered_overlap_mutect2_strelka_PCgenes_autochrom_impact.rds'))
...
saveRDS(selected, paste0(DATA_DIR, "Rdata/", model,  '_vs_', control, '__', 'filtered_overlap_mutect2_strelka_PCgenes_autochrom_impact_pathways.rds'))
