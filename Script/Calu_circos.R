...
# =============================================================================
# Calu_circos.R
# Publication-quality circos plot of the Calu-1 genomic landscape.
...
#
# Narrow gene set (050-000 baseline, union; ALL members shown only if hit by
# Amplification | Loss | HIGH-impact SV):
...
#
# Usage (defaults reproduce the Calu-1 figure when script + .rds are co-located):
#   Rscript Calu_circos.R                      # all defaults, CWD-independent
#   Rscript Calu_circos.R /path/to/calu_cnv_snv_sv.rds 000 CALU /path/to/outdir
#
#   arg 1 : path to the per-cell-model .rds (default: <scriptdir>/calu_cnv_snv_sv.rds)
#   arg 2 : condition code to filter to     (default: "000" — baseline)
#   arg 3 : output prefix + display title   (default: "CALU")
...
#
# Sibling scripts (same schema, same plotting logic): H23_circos.R, MEP_circos.R.
...
sample_ttl <- if (length(args) >= 3) args[3] else "CALU"
...
sv_cond <- sv_raw %>% filter(startsWith(._rn, barcode)) %>% select(-._rn)
if (nrow(sv_cond) == 0)
