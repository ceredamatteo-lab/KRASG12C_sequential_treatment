...
# =============================================================================
# plot_ORA_MEM_MEP.R
# Mouse counterpart of plot_ORA_H23_CALU.R for the MEF lines MEM and MEP.
#
...
#
# Plus cross-line plots (MEM vs MEP) for the kh group.
#
# Total: 2 cell lines × 2 groups × 2 variants × 2 x-axes  +  2 cross-line × 2 x-axes × 2 styles
#      = 16 + 8 = 24 PDFs in Figures/ORA/MEM_MEP/.
# =============================================================================
...
OUT_DIR       <- file.path(FIGURES_DIR, "ORA", "MEM_MEP")
...
# Cross-cell-line: MEM vs MEP as columns, kh group, de.status == "all".
plot_cross_line <- function(combined_df, group, x_var) {
  ...
  list(
    plot = ggplot(d, aes(x = .data[[x_var]], y = fct_rev(Description),
      ...
      force_panelsizes(rows = panel_heights_unit(d),
                       cols = unit(PANEL_W_CM, "cm")) +
      labs(title    = "ORA: MEM vs MEP — pathway similarity",
...
plot_cross_line_padj <- function(combined_df, group, x_var) {
  ...
  list(
    plot = ggplot(d, aes(x = .data[[x_var]], y = fct_rev(Description),
      ...
      force_panelsizes(rows = panel_heights_unit(d),
                       cols = unit(PANEL_W_CM, "cm")) +
      labs(title    = "ORA: MEM vs MEP — pathway similarity (p.adjust shown)",
...
ora_mem <- readRDS(file.path(.project_root, "ora_MEM.rds"))
ora_mep <- readRDS(file.path(.project_root, "ora_MEP.rds"))

cell_lines <- list(MEM = ora_mem, MEP = ora_mep)
...
combined <- bind_rows(
  ora_mem %>% mutate(cell_line = "MEM"),
  ora_mep %>% mutate(cell_line = "MEP")
)
for (x_var in c("FoldEnrichment","richFactor")) {
  x_suffix <- if (x_var == "richFactor") "_richFactor" else ""
